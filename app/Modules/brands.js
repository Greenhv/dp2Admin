...
export default (state = initialState, action = {}) => {
  switch (action.type) {
    ...
    case SELECT:
      return {
        ...state,
        selectedBrand: state.brands.filter(
...
export const deleteBrandAction = id => dispatch =>
  fetch(`${defaultUrl}/brands/${id}`, {
    method: "DELETE",
    headers: {
      ...customHeaders
    }
    ...
    .then(() => {
      swal("Borrado!", "El producto ha sido borrado.", "success");
...
export const updateBrand = (history, values, id) => dispatch =>
  fetch(`${defaultUrl}/brands/${id}`, {
    method: "PUT",
...
export const createBrand = (history, values) => dispatch =>
  fetch(`${defaultUrl}/brands`, {
    method: "POST",
    body: JSON.stringify(values),
    headers: {
      ...customHeaders
